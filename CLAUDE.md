...
The repository is meant to be **forked and customized** by sales engineers for their demo tenants.
...
│   ├── terraform/      # ALL Terraform configs for this tenant
│   ├── imports/        # Raw API import data (JSON)
│   └── config/         # Resource owners and labels (API-managed)
├── production/         # Production tenant (template)
├── staging/            # Staging tenant (template)
└── development/        # Development tenant (template)
```
...
# State will be stored at: s3://okta-terraform-demo/Okta-GitOps/myenv/terraform.tfstate
```
...
python3 scripts/import_oig_resources.py \
  --output-dir environments/lowerdecklabs
...
python3 scripts/import_risk_rules.py \
  --output environments/lowerdecklabs/config/risk_rules.json

# Apply risk rules to Okta
python3 scripts/apply_risk_rules.py \
  --config environments/lowerdecklabs/config/risk_rules.json \
  --dry-run  # Remove for actual apply
...
- Directory: `environments/lowerdecklabs/`
- GitHub Environment: `LowerDeckLabs` (case-insensitive match)
...
│       ├── imports/           # Raw JSON import data
│       └── config/            # API-managed configs
...
**Manual method:**
1. Navigate to environment: `cd environments/lowerdecklabs/terraform`
...
1. **Fork the repository**
   ```bash
   gh repo fork joevanhorn/okta-terraform-complete-demo --clone
   ```

2. **Remove/rename example environments**
   ```bash
   cd environments
   rm -rf lowerdecklabs  # or rename
...
4. **Import your Okta tenant**
   ```bash
   gh workflow run import-all-resources.yml -f tenant_environment=MyCompany
   ```

5. **Customize workflows**
   - Update workflow files to reference your environment names
   - Adjust approval requirements if needed

6. **Document your patterns**
   - Update this CLAUDE.md with your org-specific patterns
   - Document demo scenarios in `testing/DEMO_BUILD_GUIDE.md`

**See:** `docs/FORKING_GUIDE.md` for detailed instructions.
